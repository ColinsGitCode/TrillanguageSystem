...
-- ========================================
-- 表 49: kg_source_sync_jobs（KG 增量维护 outbox）
-- ========================================
...
-- ========================================
-- 表 50: textbook_expression_review_states（逐表达确认投影）
-- ========================================
...
-- ========================================
-- 表 51: textbook_operations（教材后台 operation）
-- ========================================
...
-- ========================================
-- 表 52: textbook_operation_events（append-only operation 事件）
-- ========================================
...
-- ========================================
-- 表 53: card_annotations（非破坏性卡片注解）
-- ========================================
...
-- ========================================
-- 表 54: card_annotation_migration_events（append-only 迁移事实）
-- ========================================
...
-- ========================================
-- 表 55: manual_tag_definitions（用户可编辑标签目录）
-- ========================================
...
-- ========================================
-- 表 56: manual_tag_assignments（标签与内容对象关联）
-- ========================================
...
-- ========================================
-- 表 58: pronunciation_documents（正文对应的读音文档）
-- ========================================
...
-- ========================================
-- 表 59: pronunciation_tokens（结构化读音单元）
-- ========================================
...
CREATE TABLE IF NOT EXISTS pronunciation_tokens (
  ...
  unit_kind TEXT NOT NULL
    CHECK (unit_kind IN ('word', 'component', 'kanji', 'punctuation', 'unresolved')),
  status TEXT NOT NULL DEFAULT 'accepted'
    CHECK (status IN ('accepted', 'unresolved', 'rejected', 'superseded')),
  source TEXT NOT NULL
    CHECK (source IN ('textbook', 'manual', 'dictionary', 'analyzer', 'rule', 'llm-proposal', 'legacy-ruby')),
...
-- ========================================
-- 表 60: pronunciation_correction_events（人工纠音事实）
-- ========================================
...
CREATE TABLE IF NOT EXISTS pronunciation_correction_events (
  ...
  token_key TEXT NOT NULL,
  event_type TEXT NOT NULL
    CHECK (event_type IN ('reading', 'boundary', 'resolve', 'reject', 'split', 'merge')),
  payload_hash TEXT NOT NULL CHECK (length(payload_hash) = 64),
...
-- ========================================
-- 表 61: local_glossary_entries（人工确认的本地中译）
-- ========================================
...
-- ========================================
-- 表 62: local_glossary_proposals（待人工确认的 LLM 中译候选）
-- ========================================
...
-- ========================================
-- 表 63: local_dictionary_entries（只读本地英日简明词典）
-- ========================================
...
-- ========================================
-- 表 64: local_glossary_lookup_events（DIC-R2 释义选择反馈，不存原句）
-- ========================================
--
-- 只记录选中的短词/短语、候选来源和人工干预动作，不记录周边原句。
-- context/snippet 不存在；其余描述字段均受 CHECK 白名单约束。
...
-- ========================================
-- 表 65-66: language_metadata_jobs / language_metadata_proposals（JLM-A0 影子提取）
-- ========================================
--
-- 任务与候选分离：一条 failed 任务代表“尚未产出结论”，不等于“没有外来语”。
-- 本域不参与 generations.content_hash，也从不读写卡片正文。
