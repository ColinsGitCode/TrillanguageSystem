...
// JLM-A2 changes the persisted Markdown shape for new cards. It is effective
// only when the sidecar metadata domain and its extractor are both enabled, so
// disabling either upstream capability restores the legacy prompt contract.
const LANGUAGE_METADATA_A2_ENABLED = LANGUAGE_METADATA_ENABLED
  && LANGUAGE_METADATA_EXTRACTION_ENABLED
  && /^(1|true|yes|on)$/i.test(String(process.env.LANGUAGE_METADATA_A2_ENABLED || '').trim());
