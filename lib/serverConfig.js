...
const CARD_ANNOTATIONS_ENABLED = /^(1|true|yes|on)$/i.test(String(process.env.CARD_ANNOTATIONS_ENABLED || '').trim());
const CARD_ANNOTATIONS_SHADOW_READ_ENABLED = /^(1|true|yes|on)$/i.test(
  String(process.env.CARD_ANNOTATIONS_SHADOW_READ_ENABLED || '').trim()
);
const CARD_ANNOTATIONS_COMPAT_WRITE_ENABLED = !/^(0|false|no|off)$/i.test(
  String(process.env.CARD_ANNOTATIONS_COMPAT_WRITE_ENABLED ?? 'true').trim()
);
...
const DEFAULT_DEEPSEEK_MODEL = sanitizeDeepSeekModelName(process.env.DEEPSEEK_MODEL) || 'deepseek-v4-pro';
