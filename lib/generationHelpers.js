...
function hasRequiredScenarioBlockFields(blocks) {
  ...
  return blocks.every((block) => {
    ...
    return /-\s*\*\*(?:中文|Chinese|中国語|中國語)\*\*\s*:/i.test(blockText)
      && /-\s*\*\*(?:使用提示|使用场景|使用場景|Usage\s*Tip|Tip)\*\*\s*:/i.test(blockText);
  });
}
...
function validateScenarioMarkdown(markdown) {
  ...
  if (!hasRequiredScenarioBlockFields(expressionBlocks)) {
    return ['scenario_phrase requires every expression block to include Chinese and usage hint lines'];
  }
