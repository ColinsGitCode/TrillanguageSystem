...
export function ReviewSummary({ title, description, items, warnings = [], actionLabel, actionPendingLabel = '处理中…', actionDisabled = false, actionPending = false, onAction, onChange }: {
  title: string;
  description?: string;
  ...
  return (
    <section className="workflow-review-summary">
      ...
      <footer><button type="button" className="primary" disabled={actionDisabled || actionPending} onClick={onAction}>{actionPending ? actionPendingLabel : actionLabel}</button></footer>
    </section>
  );
}
