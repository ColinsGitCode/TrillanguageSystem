    ...
    <aside className="textbook-track-rail" aria-label="教材 Track">
      ...
      {courses.length === 0 && (
        <div className="textbook-empty-list">
          ...
          <span>教材截图需先通过受控解析流程导入；导入后会直接进入人工校对。</span>
        </div>
      )}
      {courses.map((course) => (
        <section key={course.id}>
          <h3>{course.title}</h3>
          <p>{course.track_count ?? course.tracks?.length ?? 0} 个 Track</p>
          {(course.tracks || []).map((track) => (
            ...
            >
              ...
              <small>{statusLabel(track.status)} · {track.expression_count || 0} 条表达</small>
            </button>
          ))}
        </section>
      ))}
      <details className="textbook-advanced-intake">
        ...
        <p>正常主流程由教材解析 Skill 调用正式 API 完成导入，不要求在页面粘贴截图、路径或 OCR 结果。</p>
      </details>
    </aside>
  );
}
