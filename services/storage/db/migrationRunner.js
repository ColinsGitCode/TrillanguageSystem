...
const TEXTBOOK_WORKFLOW_TABLES = Object.freeze([
  'textbook_expression_review_states',
  'textbook_operations',
  'textbook_operation_events',
]);

const CARD_ANNOTATION_P3_TABLES = Object.freeze([
  'card_annotations',
...
const CARD_ENGAGEMENT_TABLES = Object.freeze([
...
const LOCAL_GLOSSARY_TABLES = Object.freeze([
...
const LOCAL_GLOSSARY_FEEDBACK_TABLES = Object.freeze([
  'local_glossary_lookup_events',
]);

const LANGUAGE_METADATA_TABLES = Object.freeze([
...
function assertTextbookWorkflowPostconditions(db) {
  const existing = new Set(
    db.prepare("SELECT name FROM sqlite_master WHERE type = 'table'").all().map((row) => row.name)
  );
  const missing = TEXTBOOK_WORKFLOW_TABLES.filter((table) => !existing.has(table));
  if (missing.length) throw new Error(`Textbook workflow migration missing tables: ${missing.join(', ')}`);
}

function assertCardAnnotationP3Postconditions(db) {
  const existing = new Set(
    db.prepare("SELECT name FROM sqlite_master WHERE type = 'table'").all().map((row) => row.name)
  );
...
function assertCardEngagementPostconditions(db) {
  const existing = new Set(
    db.prepare("SELECT name FROM sqlite_master WHERE type = 'table'").all().map((row) => row.name)
  );
  const missing = CARD_ENGAGEMENT_TABLES.filter((table) => !existing.has(table));
...
function assertLocalGlossaryPostconditions(db) {
  const existing = new Set(
    db.prepare("SELECT name FROM sqlite_master WHERE type = 'table'").all().map((row) => row.name)
  );
  const missing = LOCAL_GLOSSARY_TABLES.filter((table) => !existing.has(table));
...
function assertLocalGlossaryFeedbackPostconditions(db) {
  const existing = new Set(
    db.prepare("SELECT name FROM sqlite_master WHERE type = 'table'").all().map((row) => row.name)
  );
  const missing = LOCAL_GLOSSARY_FEEDBACK_TABLES.filter((table) => !existing.has(table));
  if (missing.length) throw new Error(`Local glossary feedback migration missing tables: ${missing.join(', ')}`);
}

function assertLanguageMetadataPostconditions(db) {
  const existing = new Set(
    db.prepare("SELECT name FROM sqlite_master WHERE type = 'table'").all().map((row) => row.name)
  );
  const missing = LANGUAGE_METADATA_TABLES.filter((table) => !existing.has(table));
  if (missing.length) throw new Error(`Language metadata migration missing tables: ${missing.join(', ')}`);
}
...
function runMigrations(db, options = {}) {
  ...
  assertLocalGlossaryFeedbackPostconditions(db);
  assertLanguageMetadataPostconditions(db);
...
module.exports = {
  ...
  LOCAL_GLOSSARY_FEEDBACK_TABLES,
  LANGUAGE_METADATA_TABLES,
  MANUAL_TAG_TABLES,
  ...
  assertLocalGlossaryFeedbackPostconditions,
  assertLanguageMetadataPostconditions,
  ...
  assertTextbookWorkflowPostconditions,
