...
- routes/languageMetadata.js: `GET /api/language-metadata` read-only inspection, JLM-A1 adjudication (`POST .../proposals/:id/accept`, `.../reject`, `POST .../corrections`) and idempotent manual job compensation (`POST .../jobs`), all gated by `LANGUAGE_METADATA_ENABLED` and 404 by default;
...
- `services/languageMetadata/` owns Japanese linguistic metadata proposals. `domain/foreignOriginExtraction.js` is the pure `jlm-foreign-origin-v1` validator: it re-locates an LLM-proposed katakana surface by segment plus occurrence, owns the resulting codepoint range, and rejects anything it cannot locate with one of twelve enumerated reasons. Its `proposal_key` separator is NUL, never a space. Card generation performs only a synchronous durable job enqueue after persistence; `languageMetadataJobService.js` owns the provider call in a separate worker, applies `LANGUAGE_METADATA_TIMEOUT_MS` (default 20s), retries failed jobs up to the persisted three-attempt budget, and recovers interrupted `running` jobs after restart. Initial enqueue failure is recorded in `generation_errors` and `POST /api/language-metadata/jobs` provides idempotent compensation. `LANGUAGE_METADATA_ENABLED`, `LANGUAGE_METADATA_EXTRACTION_ENABLED` and `LANGUAGE_METADATA_A2_ENABLED` all default false; A2 is effective only when the first two are also enabled. A2 changes only the new-card contract: prompt transformation stops requesting inline foreign-origin annotations and post-processing removes leaked legacy annotations before persistence. Historical Markdown/hash never changes, and every legacy `loanword-block` reader remains indefinitely because both card shapes are permanent. The metadata result never enters `/api/generate`. JLM-A1 reads foreign origins in the order newest human correction > curated dictionary > accepted LLM > pending LLM > none. Correction requests are not trusted: the server re-reads the target, content hash, codepoint range and katakana surface; a later correction creates a new accepted proposal linked by `supersedes_proposal_id`. Reads stay write-free and adjudication never writes learning, FSRS, KG, TTS, annotation, Markdown or `content_hash` data. See `Docs/Architecture/Language_Metadata_Proposal_ADR.md`, `Docs/Features/LLM_Generated_Japanese_Linguistic_Metadata_Design.md`, `Docs/TestReports/Language_Metadata_JLM_P0_DryRun_20260810.md` and `Docs/TestReports/Language_Metadata_JLM_A2_New_Card_Cutover_20260811.md`.
