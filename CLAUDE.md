...
- Local Chinese-gloss lookup is read-only and local-first: current-card exact translations, textbook expressions, confirmed local entries, versioned local dictionary entries, then exact recent-card history. Dictionary lookup receives a bounded semantic-block context and the accepted pronunciation reading when available; it ranks all matching senses instead of taking the first row, and returns bounded alternatives with visible source and confidence. It must never call DeepSeek or persist data during lookup. `LOCAL_GLOSSARY_LLM_ENABLED` controls only the explicit proposal endpoint; proposals remain pending and editable until a user accepts them into `local_glossary_entries`.
