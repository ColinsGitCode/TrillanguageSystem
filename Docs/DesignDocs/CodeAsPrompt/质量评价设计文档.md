...
### 4.6 Few-shot 机制效果指标（已落地到实验导出）
- `deltaQuality`：相对 baseline 的质量分变化
- `gainPer1kExtraTokens`：每增加 1k prompt token 带来的质量增益
- `teacherAlignmentPct`：`local_avg_quality / teacher_avg_quality`
- `teacherGapClosurePct`：相对初始 gap 的收敛比例
- `qualityStdDev`、`qualityCvPct`：轮次内质量稳定性（标准差 / 变异系数）
- `improvementRatioPct`：有效提升轮次占比
...
### 5.2 Few-shot 图表（D3）
- `round_quality_trend_*.svg`
- `round_gain_efficiency_*.svg`
- `round_alignment_stability_*.svg`
- `round_gain_tokens_scatter_*.svg`
