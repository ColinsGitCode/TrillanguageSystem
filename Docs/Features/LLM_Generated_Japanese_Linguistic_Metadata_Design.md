...
> 状态：**Draft · 第二轮评审后修订，仍不代表当前运行基线**
>
> 日期：2026-08-10（第二轮修订同日）
>
...
| 阶段 | 内容 | 退出门禁 |
|---|---|---|
| JLM-P0 | 固定样本、JSON schema、定位算法、第二次调用成本/延迟 POC | 输出可复现报告，不写生产库 |
| JLM-D1 | AI 候选、人工确认、冲突和失败状态桌面原型 | 用户逐状态确认 |
...
- [ ] 确认主生成与元数据提取采用两次独立调用；
- [ ] 确认方案 A 的 LLM 结果默认是 pending proposal；
- [ ] 确认 pending 只以“AI 候选”显示，人工确认后才转 accepted；
- [ ] 确认元数据使用独立 proposal 合同，不复用 pronunciation token `source`；
- [ ] 确认以服务器重新定位的 codepoint range 作为关联依据；
- [ ] 确认历史 Markdown/hash 不改写；
- [ ] 确认 A2 只对新卡删除 Markdown 外来语标注；
- [ ] 确认接受“新旧两种正文形态长期共存”，`loanword-block` 兼容分支不排期清理；
- [ ] 确认 curated 条目的纠错走 proposal/裁决事实，不原地改随代码发布的词典文件；
- [ ] 确认候选覆盖率与确认覆盖率分开验收；
- [ ] 确认方案 B 在 A 稳定后另行进入 POC；
- [ ] 确认方案 C 不进入自动接受链路。
