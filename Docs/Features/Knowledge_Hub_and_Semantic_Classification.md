...
3. **LLM 兜底**：规则未命中的卡片批量交给 DeepSeek（默认开，`KNOWLEDGE_CLUSTER_LLM_ENABLED`，
...
- 触发：`POST /api/knowledge/jobs/start { jobType: 'cluster' | 'index' }`（`index` 纯转换免 LLM；`cluster` 用 DeepSeek 兜底）。
...
- `cluster` 的 LLM 兜底**默认开**，会消耗 DeepSeek 配额；规则能覆盖的卡不调用 LLM。配额耗尽时
...
## 11. 学习计划（`services/storage/db/learningPlan.js`）

把「语义分类 + SRS 进度 + 难度」组装成**阶段性学习路径**，确定性、不耗 LLM。

### 11.1 结构

`GET /api/srs/plan?axis=function|topic|all`：
- 每个 active cluster = 一个**学习模块（stage）**，按平均卡片难度**由易到难**排序。
- 每个 stage 带：total / learned（SRS repetitions≥2 已掌握）/ due（今日到期）/ newCount（未跟踪）、avgDifficulty +
  difficultyLevel、easy/medium/hard 构成。
- summary：跨模块汇总 + `recommendedStage`（最易的「未完成」模块）。

SQL 一条 grouped 查询：`knowledge_clusters` JOIN `cluster_cards` LEFT JOIN `card_srs` LEFT JOIN
`knowledge_terms_index`，难度用第 10 节的 `buildDifficultyScoreSql`（与词条浏览同源）。

### 11.2 UI

Hub 左栏「学习计划 Plan」入口 → 中栏第四种模式：顶部进度汇总（学习进度 X/Y · 今日到期/新卡）+ 模块卡列表
（序号 / 名称 / 难度徽标 /「建议」标记 / 进度条 / 已掌握·到期·新 / 难度构成条 /「学这组」）。「学这组」把浏览
切到该模块对应的语义分类（设 axis + category），承接到既有浏览/复习流程。计划随轴切换重新生成。
