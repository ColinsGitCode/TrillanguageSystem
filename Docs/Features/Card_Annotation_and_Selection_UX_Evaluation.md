...
> 状态：**评估草稿 · 方向已确认，P1 复审已修订；两项 POC 已完成——迁移率（§7.1，实测可重锚 96.2%）与无头库选型/体积（§2.3.1，建议 Radix +28.5 KB）。仍待「三消费者同验 / 交互人工确认 / 切换回滚」完成后方可进入 ADR**
>
...
|---|---:|---|---|---|
| 手写基准 | — | — | — | 现状 |
| **Radix**(dropdown-menu + context-menu) | **+28.5 KB** | 2.1.24 / 2.3.7 | MIT | 稳定 |
| React Aria Components | +45.0 KB | 1.19.0 | Apache-2.0 | 稳定 |
...
**行为收益**:两库均自动施加正确 ARIA(`aria-haspopup`、`aria-expanded`、`data-state`、受管 id),这部分现为手写且易错。

**未验证项(诚实标注)**:**菜单的开合与键盘交互未能在浏览器自动化下验证**——两库均不响应合成事件(要求可信指针/键盘事件序列)。已确认库正常挂载、样式与 ARIA 正确,并用受控 `open` 验证了面板渲染,但**「点击是否顺畅打开、方向键/Escape/焦点返回是否可用」需人工实机确认**,本文不代为勾选。
...
| 无头菜单原语 → **建议 Radix** | **包装使用** | 替换手写选区工具条的菜单/浮层/键盘/焦点;保留现有 pill 视觉与 tokens。实测 **+28.5 KB gzip**、MIT、稳定版、生态最大;接入需去嵌套 3 处 CSS 规则(§2.3.1)。React Aria(+45.0 KB)a11y 更严格但体积更大;Base UI 仍 RC,暂不选 |
...
`TextPositionSelector` 的字符偏移必须对应一份**稳定、完整的整卡线性文本**。现有 `selection.ts` 只抽当前选中片段,**没有整卡投影**。ADR 前必须定义:

- `projection_version`:投影算法版本;
- **排除规则**:`rt`/`rp` 读音、`.audio-btn`、外来语 `loanword-*` 标签一律不计入可见基文;
- **空白与 Unicode 规范化**:折叠连续空白 + 固定 NFC/NFKC 口径(须与 `selection.ts` 现有归一一致);
...
## 7. 存量迁移:单位是「逻辑注解」,不是 mark 元素,更不是行(P1-4)

**复审只读审计(生产 volume,10 条 `card_highlights`)**:

- 共 **51 个独立 `<mark>`**,单条记录最多 **17** 个 mark;
- 仅 **2/10** 是 renderer v2;
- 现有 `source_hash` 与 generation `content_hash` **0/10 相同**(印证 §6.1:路径三元组才是现行身份,content_hash 未参与)。

### 7.1 POC 实测结论(2026-07-27,只读 dry-run)

对生产库副本执行重锚 dry-run,**结论:统计单位既不是行,也不是 `<mark>` 元素,而是「逻辑注解」**。
...
- **碎片合并**:先在可见基文投影上按「连续 marked 区间」还原逻辑注解,再锚定;
...
- [x] **迁移率**:已完成(§7.1)。真实数据 51 个 `<mark>` → **26 条逻辑注解**,**可重锚 96.2%**,1 条因内容漂移降级。**统计单位修正为逻辑注解**;
...
- [ ] **交互补齐**:键盘选区触发工具条、右键接管(有选区才接管、无选区放行原生菜单)、Escape、焦点返回;
      **注**:无头库的开合/键盘交互在自动化下无法验证(§2.3.1),需人工实机确认后方可勾选;
- [ ] **切换安全**:旧表退役前后的双读、逐消费者切换、回滚策略;
- [x] **依赖体积 + 视觉保真**:已完成(§2.3.1)。Radix **+28.5 KB gzip**、React Aria +45.0 KB;
      工具条零改动一致,菜单面板因 Portal 需去嵌套 3 处 CSS 规则后恢复一致;
