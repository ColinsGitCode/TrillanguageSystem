...
**教材课程**已完成 TC-D0-TC-D2 与 TC-P0-TC-P4，并于 2026-07-15 通过完整架构验收。当前具备 `/textbooks` 桌面校对工作台：Git 外 Manifest 校验、draft 导入、七张教材表、教材表达搜索、受控官方音频播放、正式 EN/JA 单句 TTS、持久化选区标红、人工 verified、显式发布到学习辅助、`textbook_en/ja` 学习单元、教材 Track 范围计划、教材复习视图，以及会同步完成态的选区派生卡任务。`TEXTBOOK_FEATURE_ENABLED` 默认开启但仍可关闭。知识图谱 2.0 已确认 KG-D0-D2，完成 KG-P0-P3 与 KG-R0；现有语料已通过 v3 hash-gated 回填并完成桌面/API 验收。本地运行环境当前只开启 KG 核心，planning 与 LLM enrichment 仍关闭；代码和示例环境的三项默认值仍全部为关闭。
...
- Architecture/Knowledge_Graph_2_0_Domain_and_Data_ADR.md：已接受的 KG-D2 领域与数据 ADR（2026-07-16 Accepted）。定义智能来源四层模型（L0 事实 / L1 确定性分析 / L2 DeepSeek 异步提案 / L3 裁决），DeepSeek 只作异步 proposal、不进同步队列不写 FSRS；11 张 `kg_*` 表 + 1 张 LA 手动入队表（表 37-48）；KP 分层身份、可逆 split/merge、unresolved 工作流、append-only lookup 幂等、`kg-lookup-signal-v1` 只读细排、`加入本次学习` 的共享 bucket 5 amendment；§21-§25 已登记 KG-P0-P3 与 KG-R0 实施、测试、真实 volume 回填、reader 性能和显式加入学习验收；
- Operations/Knowledge_Graph_2_0_Runbook.md：KG-R0 的卷级备份、稳定 Manifest 审核、hash-gated apply、投影验收、feature flag 分级启用和恢复手册；
...
学习辅助 2.0 已在全栈迁移完成后启动产品设计，当前基线为 `Features/Learning_Assistance_2_0_Design_Baseline.md`；知识图谱 2.0 已从 KG-D0 全新启动，并完成 KG-D0-D2、KG-P0-P3 与 KG-R0；仍不复活旧知识 API、旧 schema 或旧页面，且不接管学习调度状态。
