...
- Architecture/Language_Metadata_Proposal_ADR.md：**Proposed，待架构门禁确认**的 JLM-D2 ADR。定义 `language_metadata_jobs` 与 `language_metadata_proposals` 两张新表（迁移 016，未实施）、accepted 只作 status 不建投影表、`origin='human'` 承载人工修正从而压过 curated、NUL 分隔的 `proposal_key` 直接复用 JLM-P0 已测实现、五级读取优先级、含“任务创建失败不得静默丢数据”的失败矩阵、两个默认关闭的 feature flag 与 DROP 级回滚；明确不复用 `pronunciation_tokens.source`（其 `llm-proposal` 是读音域预留值且当前零使用）；本 ADR 不授权实施、不授权改 prompt、不授权历史补齐；
- Features/prototypes/jlm-d1-foreign-origin-review.html：JLM-D1 桌面原型，S1–S12 覆盖待确认、精选词典、AI 候选 pending、接受、修改后接受、拒绝、纠正精选条目、四态共存、提取进行中、提取失败、候选被服务端拒绝与正文改版 stale；合成内容，作为交互参考，**不替代用户逐状态确认**；
...
- Features/LLM_Generated_Japanese_Linguistic_Metadata_Design.md：**Draft，§11 决策已确认、JLM-P0 已完成**的日语语言学元数据 LLM 生成方案（JLM-D0）。第二轮补入全库分母（685 张 generation / 683 张含日语 / 443 张正文已含 `loanword-block` / 仅 11 张有 pronunciation projection）、curated 条目的纠错入口、A2 正文形态变更的 12 处消费方与 CR-P2 parity 口径冲突、长期双形态不排期清理，以及 extraction job 创建失败的静默丢数据分支。主卡 Markdown 生成保持独立，正文成功后再以可失败、可重试的第二次 LLM 调用提取元数据；外来语来源先进入绑定正文 hash 与位置的 pending proposal，人工确认后才成为 accepted，候选覆盖率与可靠覆盖率分开验收。文档同时修正了 4/26 仅来自 11 张 pronunciation projection 小样本、读音当前不驱动 TTS/FSRS、`content_hash` 不是 Study Item 身份键等事实，并规定历史 Markdown/hash 不改写、仅在分阶段门禁通过后对新卡移除正文外来语标注；
