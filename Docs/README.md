...
- TestReports/Local_Dictionary_DIC_R2_Observation_20260810.md：DIC-R2 上下文消歧与真实使用反馈；记录固定用例集上词性判定正确率 27.8% → 88.9%（零回退）、基线两处可复现缺陷、日语助词推断在 1,735 个サ变名词上 23.2% 的改判率、不落库整句的反馈表结构边界，以及 2 条因导入缺词性而未通过的用例；
- Architecture/Language_Metadata_Proposal_ADR.md：**Accepted 并已实施**的 JLM-D2 ADR。定义迁移 016 的 `language_metadata_jobs` 与 `language_metadata_proposals`、accepted 只作 status 不建投影表、`origin='human'` 与 `supersedes_proposal_id` 承载可版本化人工修正、NUL 分隔的 `proposal_key`、五级读取优先级、独立后台 worker（20 秒单次超时、最多 3 次重试、重启恢复）、任务创建失败的 `generation_errors` 记录与手动补偿接口，以及默认关闭的 A0/A1/A2 三层开关；A2 仅改变新卡合同，历史正文与 legacy reader 永久保留；
- Features/prototypes/jlm-d1-foreign-origin-review.html：**已确认**的 JLM-D1 桌面原型，S1–S12 覆盖待确认、精选词典、AI 候选 pending、接受、修改后接受、拒绝、纠正精选条目、四态共存、提取进行中、提取失败、候选被服务端拒绝与正文改版 stale；
- TestReports/Language_Metadata_JLM_P0_DryRun_20260810.md：JLM-P0 干跑报告；离线合同用例 16/16、单元 24/24、覆盖全部 12 种拒绝原因，真实第二次调用实测每卡约 344 输入 / 85 输出 tokens（全库补齐约 US$0.05）、成功延迟中位约 1.5s，但出现 2/8 次 120s 超时；同样 8 张卡连跑三次候选覆盖率在 64%–76% 摆动且服务端零拒绝，证明瓶颈在模型召回而非校验，据此支持 pending 默认值与 job 记录设计；全程零 SQLite 写入、零正文与 hash 改动；
- TestReports/Language_Metadata_JLM_A0_A1_Remediation_20260811.md：JLM-A0/A1 可靠性修复验收；记录主卡生成与第二次 LLM 调用解耦、持久任务重试与重启恢复、人工更正版本链、服务端目标/hash/range/surface 复核、可见失败状态、测试环境隔离与完整工程验收。结论为工程修复 PASS；A1 真实准确率与操作体验仍在观察，A2 未开始；
- TestReports/Language_Metadata_JLM_A2_New_Card_Cutover_20260811.md：JLM-A2 新卡正文切换验收；记录双形态兼容、注解锚点与 CR v3 parity、三类真实 DeepSeek/TTS 生成、Markdown/hash/磁盘一致性、40 条场景卡音频、Compose/smoke/E2E，以及“用户明确授权 A2 不等于 A1 长期观察自然达标”的边界；
- Features/LLM_Generated_Japanese_Linguistic_Metadata_Design.md：**Accepted，A0-A2 已完成、A1 观察继续**的日语语言学元数据 LLM 生成方案（JLM-D0）。主卡 Markdown 生成保持独立，正文入库后只同步创建持久任务，后台再以可失败、可重试的第二次 LLM 调用提取元数据；A2 开启后新卡不再把外来语来源写进 Markdown，历史 Markdown/hash 不改写，两种正文形态长期共存；
...
- TestReports/Pronunciation_Read_Contract_Remediation_20260804.md：注音只读合同与消费者缺陷修复报告；记录历史 GET 零写入、纠音验证、Review 局部坐标、多片段整词交互、TTS 失败重试、5 条旁路投影备份清理，以及 457 unit / 92 integration / 84 desktop E2E 的复核；仍不批准历史迁移或 Ruby 删除；
